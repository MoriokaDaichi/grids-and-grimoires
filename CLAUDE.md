...
- **Grimoire > Build Battle UI**（[BattleUISceneBuilder.cs](Assets/Editor/BattleUISceneBuilder.cs)）: `SampleScene` に戦闘HUD/ウェーブ突破（脱出選択）/報酬/**ハイドアウトのハブ（`HideoutHubPanel`）**/研究（放射状スキルツリー `ResearchTreeView`、ルート名 `ResearchRoot`）/トレードの画面ルート、`GamePhaseManager` / `PlayerInventory` / `ResearchManager` / `HideoutManager` / `TradeManager` / `EnemyRoster` / `InventoryPanel` を構築し、出撃・ハイドアウト・トレードボタンを配線する。演出/行/ノードのプレハブ（`ResearchNode` 含む）も生成。旧・単体 `EnemyStatus` GameObject は除去する。再実行可能。実行後シーン保存。
