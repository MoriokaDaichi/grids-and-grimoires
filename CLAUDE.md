...
純ロジックは `BattleFormula`（ダメージ式・Spd/Luc）/ `ManaRules`（マナ消費・回復）/ `PassiveBonusCalculator`（パッシブ集計）/ `StatusEffectController`（状態異常タイマー）/ `EndlessWaveGenerator`（エンドレスの深度スケーリング・ウェーブ生成・抽選窓）/ `MaterialLedger`（素材集計）/ `MonsterPartCatalog`（敵40体のドロップ＝固有素材の正本＝tier/属性。錬金釜・トレーダー・ハイドアウトコストが参照）/ `ResearchGraph`（放射状スキルツリーのノード定義）＋`ResearchRules`（解放判定）/ `HideoutCatalog`＋`HideoutRules`（5設備の定義・効果・変換・抽選）／`GearCatalog`（製作装備。`recipeGated` な装備はトレーダーのタスク報酬でレシピ解禁するまで作業台に出ない）/ `TradeCatalog`（交換メニュー）/ `TraderCatalog`＋`TaskRules`（複数トレーダーと依頼タスク＝タスクライン。`requires` を辿る連鎖、報酬に素材/お金/装備/レシピ/ステP）/ `DungeonEconomy`（ダンジョン入場料）／`PlayerStatSave`（手動ステータス振り分けの `SaveData` 読み書き）に切り出してあり、シーン非依存でテストできる。お金（ゴールド）は `MoneyManager`（シーンシングルトン。開始所持金・`SaveData.money` 永続化）が持ち、トレード・タスクの納金/報酬・ダンジョン入場料で使う。
...
- **未実装 / 今後**: スキルツリーのレイアウトのさらなる手調整（円盤状・重なり無しにはなっている）、ハイドアウトの見た目（等倍配置図・設備アイコン・演出）、装備のスロットUI・入替、マジックサークルの実時間経過をゲーム内に見せる演出、Character画面、AoE以外の範囲パターン。
...
- **作業台**: [GearCatalog.cs](Assets/Script/GearCatalog.cs) の装備（通常9種＋`recipeGated` の上位6種、杖/防具/アクセ ×3tier）を製作。通常品は tier ≤ 作業台Lv で解禁、`recipeGated` 品はさらにトレーダーのタスク報酬で**レシピ解禁**（`HideoutManager.RecipeUnlocked`／`unlockedGearRecipes` 永続化）が必要。ハブUIは未解放レシピを「✎レシピ未取得」のグレー行で表示。所有＝装備中扱いで `PlayerStatus.ApplyResearchDelta` に恒久ボーナス（同スロットの旧装備は置換＝ボーナスを差し引き）。`craftedGearIds` を永続化、起動時に再適用。タスク報酬の直接付与は `HideoutManager.GrantGear`（コスト・電力不要）。
...
[TraderCatalog.cs](Assets/Script/TraderCatalog.cs) が専門分野ごとに4トレーダー（両替商グレン＝結晶／精霊使いリーゼ＝エレメント／傭兵ギルド ダグ＝戦闘／蒐集家オルカ＝深層）を定義し、各自 `TradeOffer` の交換メニューと `TraderTask` の依頼を持つ。タスク種別は `DeliverItems`（納品・受取時に素材＋`deliverMoney` を消費）／`DefeatEnemies`（累計 or 種類指定の討伐数）／`ReachDepth`（最深到達）。
