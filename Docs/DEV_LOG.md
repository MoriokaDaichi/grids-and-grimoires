...
### -12. 中盤 faucet ＋ 成長ループのレート増（再検証7）（2026-09-09 その12）
Mode B 手動ディープランで「中盤のエレメント欠片・中結晶枯れで tier2 杖・作業台Lv2/Lv3・深い研究が詰まり
d12 頭打ち／成長ループのレートが +1ステP/周と遅い」を検出（`Docs/検証レポート/2026-09-09_再検証7_*.md`）。

- `ResearchGraph`: `node_*_c`/`_d`（Mega魔法の奥のステノード計10）のコスト `Md(1)`→`Sm(9)`（中結晶ゲートを外す）。
- `TraderCatalog`: orca_2b(d12)＝炎欠片×3＋闇欠片×3＋ステP+3 ／ orca_3＝炎欠片×2 ／ orca_4(d15)＝光欠片×3＋風欠片×3＋中×6。
  繰り返し討伐: `dag_grind` 25→20体・中×4＋ステP+2、`orca_grind` 40→30体・ステP+3。
- **Mode A ×20**（`_ループ_2026-09-09_cycle16_中盤faucet_x20.md`）: farm×3→push **8→12→13→15→…→20→21→21→21→21**。
  20周で **d21 頭打ち**（平均15.6、旧 x14 の 11.8 から大幅増）。HP885・Atk109・Def33・研究ステ74・タスク受領42。
  死は decisive、20周とも例外・error/warning ゼロ。d21 頭打ち＝同時4体×4×5グリッドの throughput 上限
  （貪欲は tier2/3 杖＝5×5 に到達しない）。→ 次は D（6×6グリッド）。
- EditMode 201/201 グリーン。

### -13. 深部設計 D（6×6グリッド）＋エレメント精製レート（2026-09-09 その13）
`Docs/検証レポート/2026-09-09_再検証8_サイクル17-18_D-6x6と精製レート.md`。

- **D（6×6グリッド）**（`MagicGridManager`/`SampleScene`）: `maxGridSize` 5→6。tier3 杖 → 6×6（tier2 は 5×5 据え置き）。
  `ApplyVisualSize` がセル背景の子を `w*h` まで複製（36 セル対応）。狙いは GigaFlame＋GigaFire＋単体 を同居させ
  深部の同時4体に AoE throughput で対抗すること。
- **エレメント精製レート**（`TraderCatalog`）: リーゼに `中結晶×4→<属性>の欠片×2` ／ `大結晶×1→<属性>の欠片×3` を
  全5属性ぶん追加。結晶→欠片の唯一の恒常経路（再検証7 R7-1 の中盤欠片枯れの根治）。テスト +1（202/202）。
- **ハーネス（`PlaytestAutopilot`）**: 経済パスに step 4b（余剰中結晶を Glen で大結晶・リーゼで炎/闇欠片へ変換）。
  `CollectProtectedParts` を全設備の NextCost 保護に（建造済み設備の Lv2/Lv3 強化材＝tier2 素材が変換で溶けるバグ修正）。
- **Mode A ×20 ×3**（`_ループ_1155`/`1210`/`1223`）: 精製レート＋保護素材で貪欲が **tier2 杖（樫の杖）＝5×5＝魔法8** に
  初到達（平均15.2・最大20）。ただし **5×5＋Atk112＋HP735 でも壁は d20**＝4×4 の d21 から動かない。
  → **d16〜20 の壁の主因は grid throughput ではなく AoE 火力不足**。貪欲は単体魔法中心で AoE を積まないため
  グリッドを広げても壁が動かない。**D の設計意図は Mode A では原理的に測れない → Mode B（AoE キット手詰め）で検証予定**。
- 新規所見 **S2**: 大結晶の入手が細い（貪欲・人力とも中盤 ~1 に張り付き、作業台Lv3／tier3杖／orca 大結晶変換が回らない）。
- 全プレイ（Mode A ×60 セッション）で Exception / Error / Warning ゼロ。フェイルステート健全（decisive death）。
- コミット `d9782da`(D) `2ece451`(精製＋4b) `3523d72`(保護素材)。

### -14. Mode B 手動ディープラン：D＝6×6 と成長ループの本検証（2026-09-09 その14）
`Docs/検証レポート/2026-09-09_再検証9_ModeB手動ディープラン_D6x6と成長ループ.md`。cold start 22 run（farm15＋push7）、
資源注入なし、EditMode 202/202 不変、例外・error/warning ゼロ。

- **壁の推移（push）: d10 → d14（5×5）→ d18（tier3 6×6）→ d20（研究フル）→ d21（成長ループ）**。
- **D（6×6）検証成立**: グリッドが広がるたび壁が上がり、6×6 で GigaFire＋GigaThunder＋Mega×3 の AoE throughput
  ビルドが組める（5×5 は Giga×1、4×4 は Mega＋AoE tier1 が限界）。
- **S1 確定**: Mode A で D が動かないのは貪欲が AoE を優先配置せず tier3 杖にも届かないから。人力なら +4〜6 深度。
- **サイクル18 の精製レート（中結晶→属性欠片）を実戦で確認**: 作業台Lv2→tier2 杖→5×5／作業台Lv3→tier3 杖→6×6
  のゲートを人力で突破できた（貪欲では不可）。
- **無限成長ループ稼働**（繰り返し討伐タスク13回＋大結晶→ステP）。ただしレートは緩やかで、cold start から
  20 周では d25〜30 に届かない（ブートストラップに ~15 周）。
- **★新規所見 S2（要対応）**: 大結晶の入手が中盤の最大ボトルネック（tier3 杖・作業台Lv3・orca 大結晶→ステP が
  全部大結晶ゲート、faucet が glen 中×10→大×1 のみ）。→ 次サイクルで中盤の大結晶 faucet を追加。

### -15. S2（中盤の大結晶 faucet）＋ S4（ハーネス深部対応）（2026-09-09 その15）
`Docs/検証レポート/2026-09-09_再検証10_S2-S4_大結晶faucetとハーネス深部対応.md`。

- **S2**（`TraderCatalog`）: 再検証9 の所見（大結晶が中盤の最大ボトルネック）。orca_4（d15 到達）報酬に
  大結晶×3、orca_grind（繰り返し討伐）30→25 体・大結晶 ×1→×2。テスト +1（203/203）。
- **S4**（`PlaytestAutopilot`）: Mode A で D/S2/成長ループを測れるようハーネスを深部対応に。
  (a) 魔法配置を AoE 優先（**`grid.width>=5` 限定**。初版は全グリッド適用で 3×3 が 2 枚しか載らず
  cold-start が全周 d7 フリーズ＝同サイクルで修正 `9c30a91`）(b) 設備強化 Lv2→Lv3 も追う (c) step 4b の闇欠片目標 4→7。
- **Mode A ×20（S2＋S4 後）**: 貪欲が初めて **大魔道の杖(tier3)／作業台Lv3／6×6／メガ AoE×3** に到達。
  HP725 Atk117 研究91、例外ゼロ、decisive death。
- **★ ただし壁は avg14.9 / max20 で不変**（4×4→6×6・tier1→tier3 杖・単体→メガ AoE でも横ばい）。
  ＝**d20 の壁は throughput/グリッド/杖tier ではなく、深部の敵スケーリングが「cold start ~20 周で組める
  キット」を上回る**（診断＋再検証9 の結論を裏づけ）。d20→d25 の唯一のレバーは成長ループの多数周回。
- **新規所見 S5**: cold start ~20 周で d25 に届かせるには成長ループのレートが遅すぎる（+1 深度/2周）。→ 次サイクル。
- コミット: S2 `23600d5` / S4 `f771323` / S4 修正 `9c30a91`。

### -16. S5：成長ループのレート強化 → cold start 20 周で d24（2026-09-09 その16）
`Docs/検証レポート/2026-09-09_再検証11_S5_成長ループのレート強化.md`。再検証10 の所見 S5
（S2+S4 で 6×6/tier3 杖に到達したが壁 d20 不変、成長ループが +1 深度/2周と遅い）への対応。

- `TraderCatalog`: 大結晶→ステP を 大×2→+3 / 大×5→+9（大1あたり +1.8）。中×12→中×10。
  orca_grind（繰り返し 25 体）ステP +3→+5、dag_grind（20 体）+2→+3。orca_6（d20）+6→+8、orca_8（d25）+8→+12。
- テスト `GrowthLoop_Rate_IsSteepEnough` +1（204/204）。
- **Mode A ×20**: 到達 7→24（平均16.5・最大24）。**後半10周が d17→d24 と単調増加**（サイクル21 は
  20 周でも d20 頭打ち）。HP1005 Def46（恒久ステP が毎周積まれる）。20周とも例外ゼロ・decisive death。
- **無限成長ループが Mode A でも複利で成立**＝ユーザー方針「中層farm→大結晶をステに変換くりかえし→深層」が
  ハーネス上で回っている。推移から +2〜3 周で d25〜27 見込み。
- コミット S5 `f1a2f4f`。

### -17. S7＋確認：cold start の成長ループ ×25 で d28 到達（22 周で d25）（2026-09-09 その17）
`Docs/検証レポート/2026-09-09_再検証12_S5-S7_cold-start成長ループでd25到達.md`。

- **S7**（`PlaytestAutopilot`）: `farmRuns` 3→2 / `shallowFarmCap` 7→8（壁7固定の周を減らし、farm で
  orca_2＝d8 を踏めるように）。EditMode 204/204 不変。
- **Mode A ×25**: 到達 7→**28**（平均17.2）。**run22 で d25、run25 で d28**。HP1000 Def90（再検証11 の
  Def46 から倍増＝恒久ステP が毎周積まれ続けている）。錬金釜Lv3 も建つ。25周とも例外ゼロ、decisive death。
- **深部設計（d16→d25→d30）の骨格が完成**: 診断 d20 → A+B+C 単発 d26 → **cold start 成長ループ 25 周で d28**。
  ユーザー方針「中層farm→大結晶をステに変換くりかえし→深層／討伐タスク無限」がハーネス上で回っている。
  d30 は cold start 27〜29 周で到達見込み。
- 新規所見 S9: run3〜8 が d10〜11 で 5〜6 周足踏み（Mega 研究／作業台Lv2-3 素材の開通待ち）→ 縮めれば 25 周で d30。
- コミット S7 `a723e2f`。

### -18. S9：中盤の足踏み短縮 → cold start 25 周で d29（21 周で d25）（2026-09-09 その18）
`Docs/検証レポート/2026-09-09_再検証13_S9_中盤の足踏み短縮でd29.md`。

- **S9**（`TraderCatalog`）: cold start ×25 で run3〜8 が d10〜11 で足踏み（作業台Lv2＝5×5 の欠片ゲート待ち）。
  orca_2（d8 到達）報酬に 炎の欠片×2＋ステP +1→+2、orca_2b の要件 d12→d11。EditMode 204/204 不変。
- **Mode A ×25**: 到達 8→**29**（平均18.4）。**run21 で d25、run25 で d29**（再検証12 の run22／d28 から +1 周前倒し＋最大 +1）。
  中盤の足踏みが縮んだ（run3 d10→d11）。25 周とも例外ゼロ、decisive death（d29 で mhp13%）。
- **深部設計（d16→d25→d30）は cold start の成長ループで実質達成**: 診断 d20 → A+B+C 単発 d26 →
  cold start 成長ループ 21 周で d25／25 周で d29。d30 は 26 周見込み。
- コミット S9 `24530df`。

### -19. S13＋深部設計スレッドのクローズ（2026-09-09 その19）
`Docs/検証レポート/2026-09-09_再検証14_S13_成長ループ微調整とハーネス分散.md`。

- **S13**（`TraderCatalog`）: 繰り返し討伐の結晶を各 +1（orca_grind 大×2→×3、dag_grind 中×4→×5）。EditMode 204/204 不変。
- **Mode A ×25**: 到達 8→24・平均17.5。前回（再検証13）の d29 との差は **S13 の悪化ではなくハーネス x25 の分散
  （±5 深度）**＝今回 run は序盤 RNG で錬金釜Lv3 に未到達（tier1-5 変換なし＝周またぎ結晶が細い）。
  到達推移の形は健全（8→24 単調増加）、25 周とも例外ゼロ・decisive death。S13（結晶増）はユーザー方針と一致＝維持。
- **深部設計（d16→d25→d30）スレッドをクローズ**: 診断 d20 → A+B+C 単発 d26 →
  **cold start の無限成長ループ（S2/S4/S5/S7/S9/S13）で 21〜25 周で d25〜29**。「周回するほど深くなる」がハーネス上で成立。
- 新規所見 S14: x25 分散の主因は「錬金釜Lv3 に到達したか」。`PlaytestAutopilot` のビルド優先度で前倒しすると分散が縮む。
- 残タスク: (1) Mode B で S2〜S13 込みの cold start 通しを1本（人力精度の権威ある数字）(2) S14 (3) S11（Giga 研究の段差・保留）。
- コミット S13 `20f4b53`。

### -20. S14：ハーネスの錬金釜Lv3 到達を安定化 → cold start の到達が再現性を持つ（2026-09-09 その20）
`Docs/検証レポート/2026-09-09_再検証15_S14_ハーネス分散縮小_d29再現.md`。

- **S14**（`PlaytestAutopilot`）: 再検証14 で x25 の到達分散（±5 深度）の主因が「錬金釜Lv3 に到達したか」
  と判明。錬金釜Lv3＝`L(1)+Frag(Wind,3)+魔石の欠片×3` で step 4b が Fire/Dark しか買わず Wind 欠片を
  取れなかった。→ step 4b の欠片購入を「未建設の設備 NextCost＋次に打つ杖のコスト」の属性で汎用化、
  step 9b の Lv3 優先度を 錬金釜（経済エンジン）→ 作業台 に。EditMode 204/204 不変。
- **Mode A ×25**: 到達 7→29・平均18.3。**錬金釜Lv3＋研究机Lv2 が安定して建ち**、再検証14 の悪 RNG（d24）を
  解消して再検証13 の d29 を再現＝**2 本連続で run21 で d25 / run25 で d29**。Atk129 Def100 HP1000、研究101。
- **深部設計（d16→d25→d30）の最終到達値（Mode A・2 本一致）**: 20周 d23 / 21周 d25 / 25周 d29 / d30 は 26周見込み。
  「周回するほど深くなる」が cold start から再現性を持って成立。**深部設計スレッドはクローズ。**
- コミット S14 `2c4c7e9`。

### -21. S16：cold-start 経済デッドロックの根治（無条件 faucet）（2026-09-09 その21）
`Docs/検証レポート/2026-09-09_再検証16_*.md`（検出）＋`_再検証17_*.md`（対応・確認）。

- 再検証16（Mode B 裏取り）で、S2〜S14 を入れた後も cold-start のブートストラップが
  「$14／小14／杖なし、杖⇄小結晶⇄現金⇄深度8⇄タスク が相互ロック＝1 小結晶ぶんの差で進行不能」に落ちた。
  貪欲 Mode A は物量＋全売却＋RNG で越えるが、人力 1 本＋不利シードで詰む。
- **S16**（`TraderCatalog`）: ダグ連鎖の先頭を `dag_wand`（杖製作）→ **`dag_1`（討伐10体・報酬 中×4＋40G）**。
  討伐10は 3×3/杖なしでも farm で必ず達成＝**唯一の無条件アクション**。`dag_wand` は dag_1 の次。
  `glen_1`（最初の納品）要求 小×15→×10。テスト 2 本更新/追加、EditMode 205/205。
- **確認**: Mode A ×20 回帰合格（avg17.6・例外ゼロ、むしろ序盤が速い）。**Mode B で cold-start 再確認**＝
  farm ×4 で dag_1 を無条件 claim → $130／中9 → 全設備＋杖＝**「進めない」状態なし**。デッドロック解消。
- **深部設計（d16→d25→d30）＋cold-start スレッドを完全クローズ**。
- コミット S16 `5028ada`。

### -22. Mode B 完全通しで S16 の裏取り＋改善ループの総括（2026-09-09 その22）
`Docs/検証レポート/2026-09-09_再検証18_ModeB完全通し_S16後の裏取り.md`。

- S16 込みで cold-start を Mode B で最後まで回し、**一度も「進めない」状態にならないこと**を確認
  （dag_1＝討伐10 の無条件 faucet で $160／小90／中7 が最序盤で入る）。~22 run で 5×5／push 壁 d13。
- **trajectory が Mode A / cycle19 Mode B と一致**（push 壁: 4×4 d10 → 5×5 d13）。d25/d30 の人力グラインドは
  未完だが以降は既知の道＝**Mode A の到達値（21周 d25 / 25周 d29、2 本一致）がリファレンス**。
- 既知の落とし穴（追記）: Mode B の研究割当は **furnace fuel > 0** が前提。

**改善ループ総括（cold-start・深部設計）**: 診断 d20 → A+B+C 単発 d26 → cold start の無限成長ループ
（S2/S4/S5/S7/S9/S13）で 21周 d25・25周 d29（再現性あり）→ S16 でブートストラップ・デッドロックを根治。
EditMode 180→205（+25、回帰なし）、全プレイで例外・ソフトロック ゼロ、フェイルステート全段階で decisive death。
構造的な穴は解消。残るは数値微調整（S11/S15/ハーネス研究割当）のみ。
